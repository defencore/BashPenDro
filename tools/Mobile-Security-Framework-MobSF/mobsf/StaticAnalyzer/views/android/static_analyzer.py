...
def save_app_dic_to_file(app_dic, checksum):
    ...
    for k, v in app_dic.items():
        if isinstance(v, Path):
            ...
            app_dic_serializable[k] = v  # Dictionaries are automatically serializable
        else:
            try:
                json.dumps(v)  # Check if the object can be serialized
                app_dic_serializable[k] = v
            except TypeError:
                app_dic_serializable[k] = str(v)  # Convert to string as a last resort
